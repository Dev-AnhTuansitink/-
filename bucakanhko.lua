...
NotificacaoTuanDz("NatAov Hub", "Chạy lại script nếu không hiện menu!")
    ...
    while wait() do
        pcall(function()
            ...
            or ws:FindFirstChild("Diablo") or ws:FindFirstChild("Deandre") or ws:FindFirstChild("Urban") then
                EliteHunter:SetDesc(" ✅️ | Killed: " .. progress)
            else
                EliteHunter:SetDesc(" ❌️ | Killed: " .. progress)
            end
        end)
...
spawn(function()
    pcall(function()
        while wait() do
            ...
            Bone:SetDesc("Bạn đang có : " .. tostring(bones) .. " Bones")
        end
    end)
end)
Status:AddTextBox({
...
Farm:AddSection({"Thu thập"})
-- Botão Auto Collect Chest
Farm:AddToggle({
    Name = "Nhặt rương [ tween ]",
    Description = "",
...
Setting:AddButton({
    Name = "Lưu config ",
    Description = "Lưu config đang dùng",
    Callback = function()
...
Setting:AddToggle({
	...
	Description = "bật lên để bảo vệ sức khỏe của bạn nếu mức thấp",
	-- 1. Carrega o estado salvo
...
-- 8. AUTO SPAM SKILL Z
Others:AddToggle({
    Name = "Kỹ năng tự động sử dụng cần câu",
    Description = "spam skill của cần",
    -- Carrega se estava ligado
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			r:SetDesc("Elite Trạng thái :  " .. replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress"));
		end);
	end;
end);
Others:AddToggle({
...
Others:AddSection({"Màu sắc Buso/Aura"});
...
Event:AddSection({"Sự kiện biển / chỉnh sửa"})
local z5 = {
        "Guardian",
        "PirateGrandBrigade",
        "MarineGrandBrigade",
...
Event:AddToggle({
	Name = "Di chuyển đến đảo đã spawn",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.tweenShrine = I;
...
Event:AddToggle({
	Name = "Tự động chuyển động đến điểm cao nhất",
...
if World2 or World3 then
Event:AddSection({"Setting skill ( Võ )"})
-- [[ TOGGLES MELEE ]]
Event:AddToggle({
    Name = "Skill Z",
...
Race:AddToggle({
	Name = "Cuộc đua thử tự động hoàn thành",
	Description = "",
	Default = false,
	Callback = function(I)
...
Race:AddToggle({
	Name = "Tự động tiêu diệt người chơi sau khi thử nghiệm",
...
if World3 then
Dojo:AddSection({"Dojo Nhiệm Vụ & Drago Tộc"});
Dojo:AddToggle({
	Name = "Auto làm nhiệm vụ Dojo Trainer",
	Description = "bật chế độ làm đai võ đường từ trắng sang đen",
	Default = false,
	Callback = function(I)
		_G.Dojoo = I;
	end,
...
Dojo:AddToggle({
	...
	Description = "Bật chế độ dùng thử tự động v4, bạn phải tự mình THU THẬP DI VẬT.",
	Default = false,
	Callback = function(I)
		_G.Relic123 = I;
	end,
...
Dojo:AddToggle({
	Name = "Tự động huấn luyện cuộc đua Draco",
...
-- UI E TOGGLES
Esp:AddSection({"ESP Vật phẩm/Thực thể/Đảo "});
...
Teleport:AddButton({ Name = "yêu cầu Lối vào", Description = "", Callback = function()
...
if World2 or World3 then
Get:AddToggle({
	Name = "Auto cày thông thạo súng Skull Guitar",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.AutoMatSoul = I;
...
Fruit:AddSection({"Tùy chọn trái cây"});
local J5 = {};
local function i5(I)
	local e = tostring(I);
	while true do
...
Setting:AddButton({ Name = "Chuyển sang Pirate Team", Description = "", Callback = function()
		Pirates();
	end });
Setting:AddButton({ Name = "Chuyển sang Marine Team", Description = "", Callback = function()
		Marines();
	end });
